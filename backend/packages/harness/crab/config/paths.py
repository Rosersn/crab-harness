...
class Paths:
    """
    Centralized path configuration for Crab application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. CRAB_HOME environment variable
        3. Local dev fallback: cwd/.crab-harness  (when cwd is the backend/ dir)
        4. Default: $HOME/.crab-harness
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set CRAB_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("CRAB_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("CRAB_HOME"):
            return Path(env_home).resolve()

        cwd = Path.cwd()
        if cwd.name == "backend" or (cwd / "pyproject.toml").exists():
            return cwd / ".crab-harness"

        return Path.home() / ".crab-harness"
