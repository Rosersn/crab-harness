#!/usr/bin/env bash
# chat.sh — Send a message to Crab and collect the streaming response.
#
# Usage:
...
#   CRAB_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   CRAB_URL          — Unified proxy base URL (default: http://localhost:2026)
#   CRAB_GATEWAY_URL  — Gateway API base URL (default: $CRAB_URL)
#   CRAB_LANGGRAPH_URL — LangGraph API base URL (default: $CRAB_URL/api/langgraph)
#
...
GATEWAY_URL="${CRAB_GATEWAY_URL:-$CRAB_URL}"
LANGGRAPH_URL="${CRAB_LANGGRAPH_URL:-$CRAB_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: Crab is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure Crab is running. Start it with: cd <crab-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from Crab: {data_str}", file=sys.stderr)
            sys.exit(1)
