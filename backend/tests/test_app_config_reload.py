...
from crab.config.app_config import get_app_config, reset_app_config
...
def _write_config(path: Path, *, model_name: str, supports_thinking: bool) -> None:
    path.write_text(
        yaml.safe_dump(
            {
                "sandbox": {"use": "crab.sandbox.local:LocalSandboxProvider"},
                "models": [
                    {
                        "name": model_name,
...
def test_get_app_config_reloads_when_file_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRAB_CONFIG_PATH", str(config_path))
    monkeypatch.setenv("CRAB_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    reset_app_config()
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRAB_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("CRAB_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setenv("CRAB_CONFIG_PATH", str(config_b))
