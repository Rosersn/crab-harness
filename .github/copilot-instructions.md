# Copilot Onboarding Instructions for Crab
...
Crab is a full-stack "super agent harness".
...
- `backend/packages/harness/crab/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/crab/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/crab/subagents/` - subagent registry/execution
- `backend/packages/harness/crab/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`crab.agents:make_lead_agent`)
