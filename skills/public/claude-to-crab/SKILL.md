---
name: claude-to-crab
description: "Interact with Crab AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to Crab for research/analysis, start a Crab conversation thread, check Crab status or health, list available models/skills/agents in Crab, manage Crab memory, upload files to Crab threads, or delegate complex research tasks to Crab. Also use when the user mentions crab or wants to run a deep research task that Crab can handle."
---

# Crab Skill

Communicate with a running Crab instance via its HTTP API. Crab is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Crab exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$CRAB_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$CRAB_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `CRAB_URL`          | `http://localhost:2026`                  | Unified proxy base URL             |
| `CRAB_GATEWAY_URL`  | `${CRAB_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `CRAB_LANGGRAPH_URL`| `${CRAB_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
CRAB_URL="${CRAB_URL:-http://localhost:2026}"
CRAB_GATEWAY_URL="${CRAB_GATEWAY_URL:-$CRAB_URL}"
CRAB_LANGGRAPH_URL="${CRAB_LANGGRAPH_URL:-$CRAB_URL/api/langgraph}"
```
...
Verify Crab is running:

```bash
curl -s "$CRAB_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$CRAB_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$CRAB_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$CRAB_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$CRAB_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$CRAB_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$CRAB_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$CRAB_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$CRAB_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$CRAB_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$CRAB_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$CRAB_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-crab/scripts/chat.sh "Your question here"
```
...
- If health check fails, Crab is not running. Inform the user they need to start it.
