...
from crab.agents.thread_state import AgentRuntimeContext, ViewedImageData
...
class ViewImageMiddleware(AgentMiddleware[ViewImageMiddlewareState]):
    ...
    def _should_inject_image_message(self, state: ViewImageMiddlewareState) -> bool:
        ...
        for msg in messages[assistant_idx + 1 :]:
            if isinstance(msg, HumanMessage) and getattr(msg, "name", None) == "__view_image_injection":
                return False
    ...
    def _inject_image_message(self, state: ViewImageMiddlewareState) -> dict | None:
        ...
        # The name tag lets the frontend filter it from the UI (same pattern as todo_reminder).
        human_msg = HumanMessage(content=image_content, name="__view_image_injection")
