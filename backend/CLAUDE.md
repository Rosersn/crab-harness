...
│   ├── packages/
│   │   ├── harness/           # crab-harness package (import: crab.*)
│   │   │   ├── pyproject.toml
│   │   │   └── crab/      # Agent framework (tenant-agnostic)
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/crab/` never imports from `app.*`
...
- **Harness** (`packages/harness/crab/`): Publishable agent framework package (`crab-harness`). Import prefix: `crab.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config — everything needed to build and run agents. Tenant-agnostic.
- **Platform** (`packages/platform/crab_platform/`): Multi-tenant orchestration package (`crab-platform`). Import prefix: `crab_platform.*`. Contains auth, DB models/repos, request context, Redis integration, platform config. Depends on crab-harness.
...
**Lead Agent** (`packages/harness/crab/agents/lead_agent/agent.py`):
...
**ThreadState** (`packages/harness/crab/agents/thread_state.py`):
...
Middlewares execute in strict order in `packages/harness/crab/agents/lead_agent/agent.py`:

1. **ThreadDataMiddleware** - Creates per-thread directories (`backend/.crab-harness/threads/{thread_id}/user-data/{workspace,uploads,outputs}`); Web UI thread deletion now follows LangGraph thread removal with Gateway cleanup of the local `.crab-harness/threads/{thread_id}` directory
...
2. `CRAB_CONFIG_PATH` environment variable
...
Configuration priority:
1. Explicit `config_path` argument
2. `CRAB_EXTENSIONS_CONFIG_PATH` environment variable
...
### Sandbox System (`packages/harness/crab/sandbox/`)

**Interface**: Abstract `Sandbox` with `execute_command`, `read_file`, `write_file`, `list_dir`
**Provider Pattern**: `SandboxProvider` with `acquire`, `get`, `release` lifecycle
**Implementations**:
- `LocalSandboxProvider` - Singleton local filesystem execution with path mappings
- `AioSandboxProvider` (`packages/harness/crab/community/`) - Docker-based isolation
- `E2BSandboxProvider` (`packages/platform/crab_platform/sandbox/`) - E2B cloud VM isolation with PG-backed lifecycle

**Virtual Path System**:
- Agent sees: `/mnt/user-data/{workspace,uploads,outputs}`, `/mnt/skills`
- Physical: `backend/.crab-harness/threads/{thread_id}/user-data/...`, `crab-harness/skills/`
- Translation: `replace_virtual_path()` / `replace_virtual_paths_in_command()`
- Detection: `is_local_sandbox()` checks `sandbox_id == "local"`

**Sandbox Tools** (in `packages/harness/crab/sandbox/tools.py`):
...
### Subagent System (`packages/harness/crab/subagents/`)
...
### Tool System (`packages/harness/crab/tools/`)
...
**Community tools** (`packages/harness/crab/community/`):
...
### MCP System (`packages/harness/crab/mcp/`)
...
### Skills System (`packages/harness/crab/skills/`)

- **Location**: `crab-harness/skills/{public,custom}/`
- **Format**: Directory with `SKILL.md` (YAML frontmatter: name, description, license, allowed-tools)
- **Loading**: `load_skills()` recursively scans `skills/{public,custom}` for `SKILL.md`, parses metadata, and reads enabled state from extensions_config.json
- **Injection**: Enabled skills listed in agent system prompt with container paths
- **Installation**: `POST /api/skills/install` extracts .skill ZIP archive to custom/ directory

### Model Factory (`packages/harness/crab/models/factory.py`)
...
### Memory System (`packages/harness/crab/agents/memory/`)
...
**Data Structure** (stored in `backend/.crab-harness/memory.json`):
...
### Reflection System (`packages/harness/crab/reflection/`)
...
### Embedded Client (`packages/harness/crab/client.py`)

`CrabClient` provides direct in-process access to all Crab capabilities without HTTP services. All return types align with the Gateway API response schemas, so consumer code works identically in HTTP and embedded modes.

**Architecture**: Imports the same `crab` modules that the Gateway API uses. Shares the same config files and data directories. No FastAPI dependency.
...
**Key difference from Gateway**: Upload accepts local `Path` objects instead of HTTP `UploadFile`, rejects directory paths before copying, and reuses a single worker when document conversion must run inside an active event loop. Artifact returns `(bytes, mime_type)` instead of HTTP Response. The new Gateway-only thread cleanup route deletes `.crab-harness/threads/{thread_id}` after LangGraph thread deletion; there is no matching `CrabClient` method yet. `update_mcp_config()` and `update_skill()` automatically invalidate the cached agent.
...
- If a module causes circular import issues in tests, add a `sys.modules` mock in `tests/conftest.py` (see existing example for `crab.subagents.executor`)
