...
This file is for coding agents. If the Crab repository is not already cloned and open, clone `https://github.com/ first, then continue from the repository root.

## Goal

Bootstrap a Crab local development workspace on the user's machine with the least risky path available.
...
- The Crab repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Crab.
...
- If the current directory is not the Crab repository root, clone `https://github.com/ if needed, then change into the repository root.
- Confirm the current directory is the Crab repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start Crab
